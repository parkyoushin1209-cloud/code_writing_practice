...
3. 사용한 UVM feature
   ...
   rsp를 사용해 req를 cast해주고 set_it_info를 통해 트랜잭션 id, 시퀀스 id를 복사해 item_done의 인자로 rsp를 넣어줬을 때 시퀀서가
...
3. 발생했던 문제잠
   3-1. 코드 실행 결과 트랜잭션이 빠르게 연속적으로 보내짐에 따라 인터페이스 신호가 오염(레이스 컨디션)될 수 있음을 발견했음
   3-2. reset 메서드를 한 개만 사용할 경우 페이즈 병렬화 이용 시 signal ownership이 명확하지 않아 문제가 생길 수 있음을 알 수 있었음
   
4. 해결방법
   3-1의 문제를 해결하기 위한 방법을 찾던 주 세마포를 이용한 방식을 찾아낼 수 있었음 세마포를 이용해 dut로 트랜잭션 신호가 구동되는 동안 다른 트랜잭션 신호가
